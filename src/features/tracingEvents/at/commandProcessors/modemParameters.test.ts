...
test('response from the setCommand sets the state according to the response', () => {
    const state = convertPackets([packet.command, packet.response]);

    Object.entries(packet.expected).forEach(([key, value]) => {
        console.log(key, value);
        expect(state[key as any as keyof typeof packet.expected]).toBe(value);
    });
});
